...
from model_l2loss import GPTConfig, GPT
...
scaler = torch.cuda.amp.GradScaler(enabled=(dtype == 'float16')) # defalt 2.0 may lead to overflow growth_factor=1.1, 

# optimizer
optimizer = model.configure_optimizers(optimizer_name, weight_decay, learning_rate, (beta1, beta2), rho, gamma, lr_max, device_type)
if init_from == 'resume': 
...
while True:
    ...
    if iter_num % (int(eval_interval)) == 0 and master_process:
        checkpoint = {
            'model': raw_model.state_dict(),
            'optimizer': optimizer.state_dict(),
            'model_args': model_args,
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        print(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%, scale factor: {scaler.get_scale()}")
        params = []
        ...
        for jj in range(LL):
            momentum_step = optimizer.state_dict()['state'][jj]['exp_avg']
            v_step = optimizer.state_dict()['state'][jj]['exp_avg_sq']
