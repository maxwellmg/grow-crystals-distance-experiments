...
def train_single_model(param_dict: dict):
    ...
    if data_id == "lattice":
        ...
        dataset = family_tree_dataset(p=80, num=data_size, seed=seed, device=device)
    elif data_id == "equivalence":
        dataset = mod_classification_dataset(p=100, num=data_size, seed=seed, device=device)
    elif data_id == "circle":
        dataset = modular_addition_dataset(p=59, num=data_size, seed=seed, device=device)
    else:
    ...
    if model_id == "H_MLP":
        ...
        model = ToyTransformer(vocab_size=vocab_size, d_model=embd_dim, nhead=2, num_layers=2, seq_len=input_token, use_dist_layer=True).to(device)
    elif model_id == "standard_transformer":
        model = ToyTransformer(vocab_size=vocab_size, d_model=embd_dim, nhead=2, num_layers=2, seq_len=input_token, use_dist_layer=False).to(device)
    else:
    ...
    ret_dic = {}
    ret_dic["results"] = model.train(param_dict={'num_epochs': 500, 'learning_rate': 0.001, 'train_dataloader': train_dataloader, 'test_dataloader': test_dataloader, 'device': device})
    ret_dic["model"] = model
